	...
	return (
		<Box component="form" onSubmit={(event) => { event.preventDefault(); onSubmit({ dia, inicio, fin, activa, nota: nota.trim() || undefined }); }} sx={{ display: 'grid', gap: 2 }}>
			...
			<Box sx={{ display: 'grid', gap: 2, gridTemplateColumns: { xs: '1fr', sm: 'repeat(2, minmax(0, 1fr))' } }}>
				<TextField label="Inicio" type="time" value={inicio} onChange={(event) => setInicio(event.target.value)} InputLabelProps={{ shrink: true }} fullWidth />
				<TextField label="Fin" type="time" value={fin} onChange={(event) => setFin(event.target.value)} InputLabelProps={{ shrink: true }} fullWidth />
			</Box>
