...
export class PagosService {
	...
	const { id_ss, monto, metodo_pago } = createPagoDto;
		...
		if (!solicitud) throw new NotFoundException(`Solicitud '${id_ss}' no encontrada`);

		if (solicitud.id_cliente !== id_cliente) {
			throw new UnauthorizedException('Solo el cliente propietario puede realizar el pago');
		}
		...
		if (Array.isArray(pagosExist) && pagosExist.length > 0) {
			throw new BadRequestException(`La solicitud '${id_ss}' ya tiene un pago registrado`);
		}
